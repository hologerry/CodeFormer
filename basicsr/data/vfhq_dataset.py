...
from basicsr.data.data_util import brush_stroke_mask, paths_from_folder, random_ff_mask
from basicsr.data.transforms import augment
from basicsr.utils import FileClient, get_root_logger, imfrombytes, img2tensor
from basicsr.utils.registry import DATASET_REGISTRY


def crop_square_ndarray(img, size=256):
    h, w = img.shape[:2]
...
def read_image(image_path, size=256):
    img = imread(image_path)
    img = crop_square_ndarray(img, size=size)
...
class VFHQDataset(data.Dataset):
    def __init__(self, opt):
        ...
        # inpainting mask
        # self.gen_inpaint_mask = False  # opt.get("gen_inpaint_mask", False)
        ...
        # perform corrupt
        # self.use_corrupt = False  # opt.get("use_corrupt", True)
        ...
        # if self.color_jitter_prob is not None and self.color_jitter_prob > 0:
        #     logger.info(f"Use random color jitter. Prob: {self.color_jitter_prob}, shift: {self.color_jitter_shift}")

        # to gray
        # self.gray_prob = opt.get("gray_prob", 0.0)
        # if self.gray_prob is not None and self.gray_prob > 0:
    ...
    def __getitem__(self, index):
        ...
        T = len(frame_names)
        frame_id = random.randint(0, T - 1)
        frame_name = frame_names[frame_id]
        img_path = osp.join(self.video_dir, video_name, clip_name, frame_name)
        img_gt = read_image(img_path, self.gt_size)
        ...
        #     img_in = cv2.resize(
        #         img_in, (int(self.gt_size // scale), int(self.gt_size // scale)), interpolation=cv2.INTER_LINEAR
        #     )

        #     # noise
        #     if self.noise_range is not None:
        #         noise_sigma = np.random.uniform(self.noise_range[0] / 255.0, self.noise_range[1] / 255.0)
        ...
        name = clip_name
        return_dict = {"in": img_in, "gt": img_gt, "gt_path": name, "lq_path": name, "name": name}
